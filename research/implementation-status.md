...
| `curriculum_db.py` | **Runtime reads/writes** — `load_curriculum()`, `update_knowledge()`, `load_knowledge_states()`, `generate_review_stream()` (with nexus cards + structural card mixing), `_mix_structural_cards()` (aspect + sequence + synchronic + cast + causal, activation-gated by ≥5 KI per domain, domain-diverse via window function, **type round-robin** so first 5 structural slots = one of each type, STRUCTURAL_ONLY temp flag), `get_book_prescan()` |
