...
| **Structural cards** | `generate_aspect_cards.py`, `generate_sequence_cards.py` (generation), `curriculum_db.py` `_mix_structural_cards()` (stream mixing), `review_engine.py` `record_structural_answer()` (FSRS), `AspectCard.tsx`, `SequenceCard.tsx` (components) |
...
- **Structural cards (aspect + sequence)**: `structural_cards` table (125 aspect, 8 sequence) + `structural_positions` table (529 aspect positions, 38 sequence milestones). Generation via `generate_aspect_cards.py` and `generate_sequence_cards.py` (Gemini Flash). Mixed into review stream via `_mix_structural_cards()` in `curriculum_db.py` (3 aspect + 2 sequence per batch). `POST /structural/grade` endpoint → `record_structural_answer()` in `review_engine.py` for per-position FSRS. Client: `AspectCard.tsx` (multi-signal per topic), `SequenceCard.tsx` (timeline with 2 rotating blanks).
