...
- **Structural cards (aspect + sequence)**: `structural_cards` table (~540 aspect across 7 domains, 17 sequence across 5 domains) + `structural_positions` table (~2234 positions). Generation via `generate_aspect_cards.py` and `generate_sequence_cards.py` (Gemini Flash). **Activation-gated**: aspect cards require ≥5 knowledge_items in domain, sequence cards additionally require ≥3 reviewed aspect positions. Domain-diverse selection via `ROW_NUMBER() OVER (PARTITION BY domain_id)`. Currently active: Sicily, Rome, Greece, Byzantine, Islamic. Blocked: Music, Architecture (0 KI). `STRUCTURAL_ONLY=False` (normal mixed stream — was briefly True in Session 75 for focused testing). `POST /structural/grade` endpoint → `record_structural_answer()` in `review_engine.py` for per-position FSRS. Client: `AspectCard.tsx` (multi-signal per topic), `SequenceCard.tsx` (timeline with 2 rotating blanks).
- **Entity-keyed knowledge items (Session 76, Phase 1 of entity-first architecture)**: `knowledge_entities` table parallel to `knowledge_items`, keyed by entity slug (`ent:{slug}`) instead of `{domain}:{node_id}`. Used when voice captures can't route to curriculum nodes (novel topics). Same FSRS/cached_question/key_facts schema. Voice capture entity path in `_process_voice_capture_entity_path()` fires when (a) no candidate curriculum nodes OR (b) curriculum LLM returns `node_assessments=[]`. Uses `VOICE_CAPTURE_ENTITY_PROMPT` that groups facts by entity name. Entity items render in review stream as `type:'review'` through existing `ReviewCard` with `provenance.origin='entity_capture'` and `knowledge_weight=6.0`. Background Wikidata resolution backfills `entity_id` + `wikidata_qid`. See `research/entity-first-architecture.md`.
